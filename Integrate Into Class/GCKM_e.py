...
class GaussianCopulaKmeansSynthesizer:
    ...
    def optimal_clusters_dynamic(self):
        ...
        standard_normal_df = pd.DataFrame(data=self.column_inverse_cdfs)
        range_n_clusters = range(2, 11)
        inertias = []
        
        for n_clusters in range_n_clusters:
            kmeans = KMeans(n_clusters=n_clusters, random_state=0)
            kmeans.fit(standard_normal_df)
            inertias.append(kmeans.inertia_)

        # Calculate the differences in inertia between each number of clusters
        inertia_deltas = [inertias[i] - inertias[i + 1] for i in range(len(inertias) - 1)]

        # Finding the "elbow" point where the inertia decrease is the largest
        # This method assumes that the largest drop in inertia signifies the elbow
        if inertia_deltas:
            largest_drop_index = max(range(len(inertia_deltas)), key=inertia_deltas.__getitem__)
            elbow_point = largest_drop_index + 2  # Adding 2 because cluster range starts at 2
        else:
            elbow_point = 2  # Default to the smallest number of clusters if no drops are found

        self.num_of_clusters = elbow_point
