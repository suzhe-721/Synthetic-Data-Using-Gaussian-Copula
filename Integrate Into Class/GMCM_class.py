...
class GaussianCopulaSynthesizer:
    ...
    def get_execution_times_df(self):
        ...
        fitting_time = execution_times_df.iloc[0:7]['Time (seconds)'].sum() if len(execution_times_df) > 6 else 0

        # 'Generation Time' is the time for index 7
        generation_time = execution_times_df.iloc[7]['Time (seconds)'] if len(execution_times_df) > 7 else 0

        # 'Post-Process Time' is the time for index 8
        post_process_time = execution_times_df.iloc[8]['Time (seconds)'] if len(execution_times_df) > 8 else 0
