...
class GaussianCopulaKmeansSynthesizer:
    ...
    def get_laplacian(self):
        ...
        self._stop_timer("get_laplacian")
    ...
    def optimal_clusters_dynamic(self):
      ...
      if self.num_of_clusters > 0.9*len(self.laplacian_matrix):
        self.num_of_clusters = len(self.laplacian_matrix) - self.num_of_clusters + 1
      self._stop_timer("optimal_clusters_dynamics")
    ...
    def spectral_embedding(self):
      ...
      self._stop_timer("spectral_embedding")
    ...
    def get_Kmeans(self):
      ...
      self.df = pd.DataFrame(self.spectral_embedding_result)
      ...
      for cluster_idx in range(self.num_of_clusters):
        cluster_data = self.df[self.kmeans_labels == cluster_idx]
    ...
    #     self._stop_timer("generate_data")
    ...
    def generate_synthetic_data(self, num_rows):
        ...
        # Special handling for synthetic columns with specific formats
        for column in self.special_numeric_series_columns:
            prefix = self.special_column_prefixes[column]
            number_length = self.digit_counts[column]
            synthetic_df[column] = [prefix + self.generate_random_number(number_length) for _ in range(num_rows)]

        for column in self.hyphenated_numeric_columns:
            format_pattern = self.hyphenated_format.get(column, [])
            synthetic_df[column] = [self.generate_random_hyphenated_number(format_pattern) for _ in range(num_rows)]
    ...
    def get_execution_times_df(self):
        ...
        fitting_time = execution_times_df.iloc[0:11]['Time (seconds)'].sum() if len(execution_times_df) > 6 else 0


        generation_time = execution_times_df.iloc[11]['Time (seconds)'] if len(execution_times_df) > 7 else 0

        post_process_time = execution_times_df.iloc[12]['Time (seconds)'] if len(execution_times_df) > 8 else 0
